...
describe('Tool Input Fuzz Tests', () => {
  ...
  describe('Layer 2: Logic Invariants', () => {
    const fuzzableTools = regularTools.filter((t) => {
      return !LOGIC_SKIP.has(t.name) && !isCamofoxTool(t.name);
    });

    if (fuzzableTools.length === 0) {
      it('has no tools eligible for logic fuzzing', () => {
        expect(fuzzableTools.length).toBe(0);
      });
    } else {
  ...
  describe('Layer 3: Response Formatter Safety', () => {
    ...
    if (toolsWithFormatters.length === 0) {
      it('has no tools with response formatters', () => {
        expect(toolsWithFormatters.length).toBe(0);
      });
    } else {
